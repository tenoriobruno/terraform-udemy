...
resource "aws_route_table" "route_table" {
  ...
  tags = {
    Name = "route-table-terraform-${var.env}"
  }
}
...
resource "aws_vpc_security_group_ingress_rule" "sg_ingress" {
  ...
  to_port           = 22
  tags = {
    Name = "sg-ingress-terraform-${var.env}"
  }
}
...
resource "aws_vpc_security_group_egress_rule" "securty_group" {
  ...
  tags = {
    Name = "sg-egress-terraform-${var.env}"
  }
}